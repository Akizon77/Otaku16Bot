...
namespace Otaku16.Tools
{
    public class Telegram
    {
        ...
        public static string GetName(long? chatid)
        {
            ...
            // 优先使用用户名作为聊天名称
            if (c.Username != null) return $"@{c.Username}";
            // 尝试使用名和姓作为聊天名称
            if (c.FirstName != null && c.LastName != null) return $"{c.FirstName} {c.LastName}";
            // 如果只有名或只有姓，分别使用它们作为聊天名称
            if (c.FirstName != null) return $"{c.FirstName}";
            if (c.LastName != null) return $"{c.LastName}";
            // 如果上述都不存在，使用聊天标题作为名称
